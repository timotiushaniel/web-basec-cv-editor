...
if(isset($_POST["lg_id"]) && !empty($_POST["lg_id"])){
    ...
    if(empty($language_err) && empty($language_test_err) && empty($Language_proficient_err) && empty($score_err) && empty($tanggal_err)){
        ...
        $sql = "UPDATE language SET pd_id=?, language=?, language_test=?, Language_proficient=?, score=?, date=? WHERE lg_id=?";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            $pd_id = $pd_id_ses;
            mysqli_stmt_bind_param($stmt, "isssssi", $pd_id, $language, $language_test, $Language_proficient, $score, $tanggal, $lg_id);
