...
if (isset($_POST['upload'])) {
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        if (in_array($fileactualext, $allowed)) {
            if ($fileerror === 0) {
                if ($filesize < 2000000) {
                    $filenamenew = "profile-user"."update".$UserId.".".$fileactualext;
                    $filedestination = '../assets/profile-picture/'.$filenamenew;
                    move_uploaded_file($filetmpname, $filedestination);
                    $sql = "UPDATE picture SET picture=? WHERE us_id=?";
                    ...
                    if($stmt = mysqli_prepare($conn, $sql)){
                        ...
                        if(mysqli_stmt_execute($stmt)){
                            ...
                            header("location: ../assets/icon/crop.php");
                            exit();
                        } else{
...
<body>
    ...
    <h2 class="alert alert-primary text-center mt-3">Personal data</h2><hr>
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" enctype="multipart/form-data">
            <table width=100%>
                <tr>
                    <td><input type="file" class="form-control-file border" id="Foto" name="Foto"></td>
